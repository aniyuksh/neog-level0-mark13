...
function isPallind(str) {
  var reversedString = reversed(str);
...
function pallindAllFormat(date) {
  var dateFormatList = getDAteFormats(date);
  var palindromeList = [];

  for (var i = 0; i < dateFormatList.length; i++) {
    var result = isPallind(dateFormatList[i]);
...
function nextPallind(date) {
  ...
  while (1) {
    ...
    var resultList = pallindAllFormat(dateStr);
...
function clickHandler(e) {
  ...
  if (bdayString !== '') {
    ...
    var list = pallindAllFormat(dateStr);
    ...
    if (!isPalindrome) {
      const [ctr1, nextDate] = nextPallind(date);
